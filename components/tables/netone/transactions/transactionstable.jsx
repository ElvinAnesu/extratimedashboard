...
export default function NetoneTransactionTable() {
	...
	const getTransactions = async (page = 1) => {
		...
		const res = await fetch(
			`/api/netone/transactions?page=${page}&pageSize=${PAGE_SIZE}`,
			{
				method: "GET",
					...
					<table className="w-full">
						<tbody>
							...
							{transactions.map((transaction, i) => (
								<tr
									className="bg-gray-200 text-xs py-1 border-b border-gray-300"
									key={i}
								>
									...
									<td
										className={`px-4 ${
											transaction.cleared ? "text-green-600" : "text-amber-600"
										} hidden md:block`}
									>
