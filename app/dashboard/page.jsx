...
export default function Dashboard() {
  ...
  const [fetchingoutstainging, setFetchingoutstanding] = useState(false);
  ...
  const getMalcolmtotals = async (_id) => {
    const response = await fetch(
      "/api/airtimetransactions/totals/supervisors",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const getOutsaningcollections = async () => {
    ...
    if (data.success) {
      setOutandingcollections(data.total);
    ...
    <div className="flex flex-col gap-8">
      <div className="flex flex-col p-4 gap-4 md:flex-row items-center justify-end gap-4">
        <Link href="#">
          <DashboardCard
            value={
              fetchingoutstainging
                ? "loading..."
                : `USD${outstandingcollections.toFixed(2)}`
            }
        ...
        <DashboardCard
          value={
            fetchingsales
              ? "loading..."
              : `USD${(
                  collectionstodaymalcolm +
                  collectionstodaydenny +
                  collectionstodayjames +
                  collectionstodaytinashe
                ).toFixed(2)}`
          }
      ...
      <div className="flex flex-col md:grid md:grid-cols-3 w-full h-full gap-4 bg-gray-200 p-4 rounded">
        <div className="flex flex-col gap-2 items-center justify-center">
          ...
          <ServicePerfomancePieChart
            _labels={["James", "Tinashe", "Denny", "Malcolm"]}
            _data={[
              outsandingjames,
              outsandingtinashe,
              outsandingdenny,
        ...
        <div className="flex flex-col gap-2 items-center justify-center">
          ...
          <ServicePerfomancePieChart
            _labels={["James", "Tinashe", "Denny", "Malcolm"]}
            _data={[
              agentperfomancejames,
              agentperfomancedtinashe,
              agentperfomancedenny,
              agentperfomancedmalcolm,
            ]}
          />
        </div>
      </div>
